...
class MultiheadAttention(nn.Module):
    ...
    def attention_ops(self, q, k, v, key_padding_mask=None, attn_mask=None, rel_pos=None, is_causal=False):
        # 如果未显式开启 flash_attention，或者环境中没有可用的 flash_attn_func，
        # 或者存在相对位置编码（当前实现不支持与 flash_attn 同时使用），
        # 则统一走常规的 scaled dot-product attention 分支。
        use_flash = (
        ...
        if not use_flash:
            ...
            if key_padding_mask is not None:
                ...
                # 兼容长序列场景：当 key_padding_mask 长度与注意力矩阵的 s 维不一致时，
                # 跳过 mask，避免因形状不匹配导致运行时错误。
                if key_padding_mask.size(1) == attn_weights.size(-1):
            ...
            # 使用 flash-attn 分支（仅在环境正确安装 flash-attn 时启用）
