...
class DilatedAttention(MultiheadAttention):
    ...
    def sparse_to_dense(self, out, lse, ratio):
        # float16 下 -1e8 会溢出（范围约 ±65504），用 -65000 等效 -inf 且安全
        LSE_NEG_INF = -65000.0
        head_padding = padding_to_multiple_of(self.num_heads, ratio)

        if head_padding > 0:
            out = F.pad(out, (0, 0, 0, head_padding), value = 0.)
            lse = F.pad(lse, (0, 0, 0, head_padding), value = LSE_NEG_INF)

        # 保证 (r h) 维可被 ratio 整除，否则 rearrange 会报错（如 1219 不能被 4 整除）
        r_h_dim = lse.size(1)
        pad_r_h = (ratio - (r_h_dim % ratio)) % ratio
        if pad_r_h > 0:
            lse = F.pad(lse, (0, 0, 0, pad_r_h), value=LSE_NEG_INF)
        ...
        lse = lse.masked_fill_(lse == 0, LSE_NEG_INF)
    ...
    def scattering(self, outs, lses, seq_len, bsz, offset=0):
        ...
        for dr, o, lse in zip(drs, outs, lses):
            ...
            # 非 Flash 路径下 lse 可能是 (b*h) 而非 (b*n)，或最后一维为 attention 的 s 维（未压成 1），需统一
            if lse.dim() >= 4 and lse.size(-1) > 1:
                lse = lse.sum(dim=-1, keepdim=True)
            if lse.size(0) != o.size(0):
                n_seg = o.size(0)  # b*n
                group_size = lse.size(0) // n_seg  # (b*h)/(b*n)
                need_el = n_seg * group_size * self.num_heads * lse.size(2) * 1
                if lse.numel() != need_el and lse.size(-1) > 1:
                    lse = lse.sum(dim=-1, keepdim=True)
                lse = lse.reshape(
                    n_seg, group_size, self.num_heads, lse.size(2), 1
                ).mean(dim=1)
    ...
    def forward(
        ...
        # 当未开启 Flash Attention 或环境中没有可用的 flash_attn 时，
        # 回退到标准多头注意力实现，放弃 LongNet 的稀疏 / 分段逻辑，
        # 以保证数值和维度稳定，而不会在 dilated 路径里因 LSE 形状出错。
        use_flash = (
            getattr(self.args, "flash_attention", False)
