...
def _is_slide_filename(name: str) -> bool:
    """OpenSlide 常见切片后缀（大小写不敏感）。"""
    n = name.lower()
    return n.endswith((".tif", ".tiff", ".svs"))


def list_tif_paths(directory: str, *, recursive: bool = False) -> List[str]:
    """列出目录下 .tif/.tiff/.svs 路径，按路径排序以保证确定性。"""
    ...
    if recursive:
        for root, _dirs, files in os.walk(directory):
            for name in files:
                if _is_slide_filename(name):
                    out.append(os.path.join(root, name))
    else:
        for name in sorted(os.listdir(directory)):
            path = os.path.join(directory, name)
            if os.path.isfile(path) and _is_slide_filename(name):
                out.append(path)
    out.sort()
    return out
